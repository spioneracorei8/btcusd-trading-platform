...
// GetENV returns the trimmed value of an environment variable, or
// defaultValue when it is unset or blank.
//
...
func GetENV(key, defaultValue string) string {
	value, ok := LookupENV(os.LookupEnv, key)
	if !ok {
		return defaultValue
	}
	return value
...
// non-blank value. Taking the lookup as a parameter lets tests supply an
