...
1. **Binding.** The api container publishes on `127.0.0.1` and on this host's
   tailnet address, and on nothing else. `deploy/docker-compose.prod.yml` uses
   `${TAILSCALE_IP:?...}`, so an unset value aborts the command rather than
   expanding to `:8080:8080` and binding every interface — the failure that
   would not be visible in `docker compose ps`.
...
`deploy/docker-compose.yml` + `deploy/docker-compose.prod.yml` with
`.env.example`:

```console
$ TAILSCALE_IP= docker compose -f deploy/docker-compose.yml \
    -f deploy/docker-compose.prod.yml --env-file .env.example config
error while interpolating services.api.ports.[]: required variable TAILSCALE_IP
is missing a value: set TAILSCALE_IP in .env to this host's tailnet address;
see deploy/README.md §5
```

With it set, the api resolves to exactly two published addresses and postgres
to one:
...
- `deploy/README.md` §7 — the firewall
