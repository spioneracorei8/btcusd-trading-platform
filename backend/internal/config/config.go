...
// Sentinel errors returned (wrapped, and joined together) by Load.
var (
...
const (
	...
	defaultMarketType       = string(domain.MarketTypeSpot)
...
func LoadFrom(lookup LookupFunc) (*Config, error) {
	...
	cfg := &Config{
		...
		Database: Database{
			...
			MaxConns:       int32(l.optionalInt("DATABASE_MAX_CONNS", defaultDatabaseMaxConns, 1, 1000)),
			ConnectTimeout: defaultConnectTimeout,
		},
		Market: Market{
			Symbol:        l.optionalString("MARKET_SYMBOL", defaultMarketSymbol),
			Type:          l.marketType("MARKET_TYPE"),
			Timeframes:    l.timeframes("MARKET_TIMEFRAMES"),
			FeeTakerPct:   l.feePct("FEE_TAKER_PCT"),
			SlippageTicks: l.optionalInt("SLIPPAGE_TICKS", defaultSlippageTicks, 0, 1000),
		},
		Notify: Notify{
...
func (l *loader) invalidf(key, format string, args ...any) {
	l.invalid = append(l.invalid, fmt.Errorf("%w: %s: %s", ErrInvalidEnv, key, fmt.Sprintf(format, args...)))
}
...
func (l *loader) appEnv(key string) AppEnv {
	v, ok := l.get(key)
	if !ok {
		l.missing = append(l.missing, key)
		return ""
	}
	env := AppEnv(strings.ToLower(v))
	if !env.Valid() {
		l.invalidf(key, "%q is not one of %q, %q", v, EnvDev, EnvProd)
		return ""
	}
	return env
}
...
func (l *loader) marketType(key string) domain.MarketType {
	v := l.optionalString(key, defaultMarketType)
	mt, err := domain.ParseMarketType(v)
	if err != nil {
...
func (l *loader) timeframes(key string) []domain.Timeframe {
	raw := l.optionalString(key, defaultMarketTimeframes)
...
func (l *loader) feePct(key string) decimal.Decimal {
	v := l.optionalString(key, defaultFeeTakerPct)
...
// with errors.Is against ErrMissingEnv and ErrInvalidEnv.
